...
export default async function handler(req, res) {
  ...
  try {
    ...
    // إعداد أوامر yt-dlp مع الجودة المحددة
    let ytdlpArgs = [
      url,
      '-o', filepath,
      '--no-playlist',
    ...
    // تحديد الجودة حسب المنصة
    if (platform === 'youtube') {
      if (quality === 'highest') {
        ytdlpArgs.push('--format', 'bestvideo+bestaudio/best');
      } else if (quality === 'lowest') {
        ytdlpArgs.push('--format', 'worstvideo+worstaudio/worst');
      } else {
        // جودة محددة مثل 720p, 1080p
        ytdlpArgs.push('--format', `best[height<=${quality.replace('p', '')}]/best`);
      }
    } else {
      // باقي المنصات
      ytdlpArgs.push('--format', 'bestvideo+bestaudio/best');
