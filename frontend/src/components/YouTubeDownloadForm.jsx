...
const YouTubeDownloadForm = ({ language }) => {
  ...
  const translations = {
    ar: {
      ...
      format: 'نوع الملف',
      downloadFrom: 'تحميل من يوتيوب',
      pasteLink: 'الصق رابط يوتيوب واختر الإعدادات',
      ...
      enterValidUrl: 'يرجى إدخال رابط يوتيوب صحيح',
    ...
    en: {
      ...
      format: 'File Format',
      downloadFrom: 'Download from YouTube',
      pasteLink: 'Paste the YouTube link and choose your settings',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.url.trim() || (!formData.url.includes('youtube.com') && !formData.url.includes('youtu.be'))) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3 mb-2">
                {formats.map((format) => (
                  <button
                    ...
                    className={`p-3 rounded-lg border transition-colors font-medium ${formData.format === format ? 'border-red-500 bg-red-50 text-red-700' : 'border-gray-300 text-gray-700 hover:border-red-300 hover:bg-red-50'}`}
                  >
          ...
          {isLoading && (
            <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden mb-4">
              <div
                className="h-full bg-gradient-to-r from-red-500 to-pink-600 rounded-full transition-all duration-500 ease-out shadow-lg"
          ...
          <div className="pt-4">
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full py-3 px-6 rounded-lg text-lg font-medium transition-colors ${isLoading ? 'bg-gray-400 cursor-not-allowed text-white' : 'bg-red-600 hover:bg-red-700 text-white'}`}
            >
              {isLoading ? (
...
export default YouTubeDownloadForm; 