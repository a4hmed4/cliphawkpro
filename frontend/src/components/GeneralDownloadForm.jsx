...
const GeneralDownloadForm = ({ language }) => {
  ...
  const translations = {
    ar: {
      ...
      format: 'نوع الملف',
      downloadFrom: 'تحميل من منصات أخرى',
      ...
      enterValidUrl: 'يرجى إدخال رابط صحيح',
    ...
    en: {
      ...
      format: 'File Format',
      downloadFrom: 'Download from Other Platforms',
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3 mb-2">
                {formats.map((format) => (
                  <button
                    ...
                    className={`p-3 rounded-lg border transition-colors font-medium ${formData.format === format ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-300 text-gray-700 hover:border-green-300 hover:bg-green-50'}`}
                  >
          ...
          {isLoading && (
            <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden mb-4">
              <div
                className="h-full bg-gradient-to-r from-green-500 to-teal-600 rounded-full transition-all duration-500 ease-out shadow-lg"
          ...
          <div className="pt-4">
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full py-3 px-6 rounded-lg text-lg font-medium transition-colors ${isLoading ? 'bg-gray-400 cursor-not-allowed text-white' : 'bg-green-600 hover:bg-green-700 text-white'}`}
            >
              {isLoading ? (
...
export default GeneralDownloadForm; 