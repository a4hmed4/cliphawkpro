...
const InstagramDownloadForm = ({ language }) => {
  ...
  const translations = {
    ar: {
      ...
      videoUrl: 'رابط فيديو انستغرام',
      quality: 'جودة الفيديو',
      audioQuality: 'جودة الصوت',
      format: 'نوع الملف',
      downloadFrom: 'تحميل من انستغرام',
      pasteLink: 'الصق رابط انستغرام واختر الإعدادات',
      ...
      enterValidUrl: 'يرجى إدخال رابط انستغرام صحيح',
      errorDownloading: 'حدث خطأ أثناء التحميل'
    },
    en: {
      downloadVideo: 'Download Video',
      videoUrl: 'Instagram Video URL',
      quality: 'Video Quality',
      audioQuality: 'Audio Quality',
      format: 'File Format',
      downloadFrom: 'Download from Instagram',
      pasteLink: 'Paste the Instagram link and choose your settings',
      ...
      enterValidUrl: 'Please enter a valid Instagram URL',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.url.trim() || !formData.url.includes('instagram.com')) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3 mb-2">
                {formats.map((format) => (
                  <button
                    ...
                    className={`p-3 rounded-lg border transition-colors font-medium ${formData.format === format ? 'border-pink-500 bg-pink-50 text-pink-700' : 'border-gray-300 text-gray-700 hover:border-pink-300 hover:bg-pink-50'}`}
                  >
          ...
          {isLoading && (
            <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden mb-4">
              <div
                className="h-full bg-gradient-to-r from-pink-500 to-orange-400 rounded-full transition-all duration-500 ease-out shadow-lg"
          ...
          <div className="pt-4">
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full py-3 px-6 rounded-lg text-lg font-medium transition-colors ${isLoading ? 'bg-gray-400 cursor-not-allowed text-white' : 'bg-pink-600 hover:bg-pink-700 text-white'}`}
            >
              {isLoading ? (
